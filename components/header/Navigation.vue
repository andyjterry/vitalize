<template>
  <header class="absolute inset-x-0 top-0 z-50  bg-white/90 sticky top-0" style="">
    ...
    >
      ...
      <div class="hidden lg:flex lg:gap-x-12">
        <a
          ...
          :key="item.name"
          :href="item.href"
          class="text-xl font-semibold leading-6 text-gray-700 hover:text-orange-500"
        >
    ...
    <Dialog as="div" class="lg:hidden" @close="mobileMenuOpen = false" :open="mobileMenuOpen">
      ...
      <DialogPanel
        class="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white px-2 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10"
        ...
        <div class="mt-6 mx-3 flow-root">
            <div class="space-y-2 py-2">
              <a
                ...
                :key="item.name"
                :href="item.href"
                class="-mx-3 flex flex-col space-y-2 px-3 py-2"
              >
                <span v-if="item.main" class=" text-lg font-semibold leading-7 text-gray-900 hover:bg-gray-5">{{ item.name }}</span>
                <span v-if="item.description" class="text-xs text-gray-500/70">{{ item.description }}</span>
              </a>
            </div>
